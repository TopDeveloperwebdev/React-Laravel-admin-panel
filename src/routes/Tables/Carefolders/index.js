...
class Carefolders extends Component {
	...
	generatePdf(documentsLen) {
		let pageH = 1123;
		setTimeout(() => {
			this.formate(documentsLen, pageH);
			...
			pdf.fromHTML(
				...
				for (let j = 1; j < pages + 1; j++) {
					let horizontalPos = pageWidth;  //Can be fixed number
					let verticalPos = pageHeight - 10;  //Can be fixed number
					pdf.setPage(j);
					pdf.text("my header text", 10, 50)
					...
					pdf.addImage(baseB6, '', 10, 50, 50, 50)

					pdf.text(`Seilte${j} von ${pages}`, horizontalPos, verticalPos, {
						align: 'right'  //Optional text styling});
					})
				}
				pdf.save('Test.pdf');
			}, margins
